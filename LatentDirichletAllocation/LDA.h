...
private:
	LDA(void);
public:
	LDA(const std::string &file_bow, const std::string &file_vocabulary, const int &K, const int &seed = -1, const double &alpha_total_mass_ = 1.0, const double &beta_total_mass_ = 20.0);
	virtual ~LDA(void);
